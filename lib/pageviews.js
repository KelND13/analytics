...
//     let self = this;
//     self.getWhatsNewsArticles(function(err, res) {
//         if (err) return callback(err);
//         self.getPackageContents(sbids, packages, function(err, allsbids) {
//             if (err) return callback(err);
    ...
    request(options, function(err, res, body) {
        ...
        for (var p = 0; p < packages.length; p++) {
            for (var c = 0; c < currentpackages.length; c++) {
                if (packages[p].key === currentpackages[c].key) {
                    packagescontents.push(packages[p].contents);
                    break;
                }
            }
        }
        ...
        // Write for loop to loop over packages, and check if the package ID matches one in our list of packages 
        // Put the contents of each package match into an array.

        // Combine allsbids with package contents 
    });
}
